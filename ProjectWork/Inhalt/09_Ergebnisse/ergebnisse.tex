...
\begin{figure}[ht]
	\begin{center}
		...
		\caption{Entwicklung von \textit{text} und \textit{mAP} während dem Trainingsprozess von \textit{YOLO}}
		\label{yolo_result}
	\end{center}
\end{figure}

Wie beim \textit{SSD} erreicht das Modell relativ frühzeitig nach bereits etwa 4000 Batches eine \textit{mAP} von 79\%, woraufhin das Training vor Erreichung der durch die Dokumentation nahegelegte maximale Anzahl an Batches abgebrochen wurde. Das kann darauf zurückgeführt werden, dass es sich bei den gelabelten Klassen um Objekte des gleichen Typs handelt. Tabelle \ref{table:yoloresults} zeigt die \textit{mAP} für jede gelabelte Klasse:
...
Auffällig niedrig ist die \textit{mAP} der Klassen \textit{Pepsi Cola Groß} und \textit{Pepsi Cola Klein}. Ein reiner Wertevergleich gibt jedoch keinen guten Einblick in den tatsächlichen Unterschied der zwei trainierten Modelle. Deshalb wird im folgenden das Detektionsverhalten auf den gleichen Bildern wie zuvor bei SSD evaluiert:
...
\end{figure}
 ...
 \begin{figure}[ht]
 	\subfigure[überbeleuchtet]{\includegraphics[width=0.30\textwidth]{Bilder/yolo_beleuchtung1.jpg}}\hfill
 	\subfigure[normal]{\includegraphics[width=0.30\textwidth]{Bilder/yolo_beleuchtung2.jpg}}\hfill
...
Um dem entgegen zu wirken, wurde ein Bufferkonzept in einem parallelem Thread realisiert, der einzelne Frames zeitgleich zur Inferenz anfragt und zwischenspeichert. Ist der Buffer voll, so werden nach dem \textit{First In First Out} Verfahren die älteren Frames verworfen. Dadurch konnte die FPS Anzahl von \textit{SSD} von maximal 18 auf die vollen 30 gesteigert werden. Dadurch sollten sämtliche Änderungen in der Umgebung rechtzeitig vom Objektdetektor erkannt werden. 
